...
def prepare_images_for_saving(images_tensor, resolution, grid_size=4, range_type="neg1pos1"):
    if range_type != "uint8":
        images_tensor = torch.nan_to_num(images_tensor)  # NaN/inf 제거
...
#     resolution: int
#     grid_size: int (grid_size x grid_size로 시각화)
#     """
#     if range_type != "uint8":
#         images_tensor = (images_tensor * 0.5 + 0.5).clamp(0, 1) * 255
#
...
#
#     if B < expected:
#         pad = torch.zeros(expected - B, *images_tensor.shape[1:], dtype=images_tensor.dtype)
#         images_tensor = torch.cat([images_tensor, pad], dim=0)
#
#     images = images_tensor[:expected].permute(0, 2, 3, 1).numpy().astype("uint8")
#     grid = images.reshape(grid_size, grid_size, resolution, resolution, 3)
#     grid = np.swapaxes(grid, 1, 2).reshape(grid_size * resolution, grid_size * resolution, 3)
#     return grid
...
#         pad_len = expected - data.shape[0]
#         pad = np.zeros(pad_len, dtype=data.dtype)
#         data = np.concatenate([data, pad])
#
#     data = data[:expected].reshape(grid_size, grid_size)
#
#     fig = plt.figure(figsize=(20, 20))
