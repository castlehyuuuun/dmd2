from diffusion.DMD2_main.third_party.edm.training.networks import EDMPrecond
from diffusion.DMD2_main.main.edm.edm_network import get_imagenet_edm_config
...
def sample(accelerator, current_model, args, model_index):
    ...
    while len(all_images_tensor) * args.eval_batch_size * accelerator.num_processes < args.total_eval_samples:
        ...
        eval_images = current_model(noise * args.conditioning_sigma, timesteps * args.conditioning_sigma,
                                    one_hot_labels)
    ...
    if accelerator.is_main_process:
        ...
        eval_images_grid = all_images[:grid_size * grid_size].reshape(grid_size, grid_size, args.resolution,
                                                                      args.resolution, 3)
        eval_images_grid = np.swapaxes(eval_images_grid, 1, 2).reshape(grid_size * args.resolution,
...
def calculate_inception_stats(all_images_tensor, evaluator, accelerator, evaluator_kwargs, feature_dim, max_batch_size):
    ...
    num_batches = ((len(all_images_tensor) - 1) // (
                max_batch_size * accelerator.num_processes) + 1) * accelerator.num_processes
    ...
    for i in tqdm(range(num_batches // accelerator.num_processes), unit='batch',
...
def evaluate():
    ...
    # initialize wandb 
    if accelerator.is_main_process:
        run = wandb.init(config=args, dir=args.folder,
    ...
    while True:
        ...
        for checkpoint in new_checkpoints:
            ...
            pred_mu, pred_sigma = calculate_inception_stats(all_images_tensor, evaluator,
                                                            accelerator, evaluator_kwargs, feature_dim,
                                                            args.max_batch_size,
                                                            )
